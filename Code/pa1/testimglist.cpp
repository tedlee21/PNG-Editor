...
void MyTest() {
  PNG inpng = PNG(5, 5);
  for (int y = 0; y < inpng.height(); y++) {
    HSLAPixel *pixel = inpng.getPixel(0,y);
    pixel->h = 5.1;
    pixel->l = 0.5;
    pixel->s = 1;
    pixel = inpng.getPixel(1,y);
    pixel->h = 120.3;
    pixel->l = 0.5;
    pixel->s = 1;
    pixel = inpng.getPixel(2,y);
    pixel->h = 5.4;
    pixel->l = 0.5;
    pixel->s = 1;
    pixel = inpng.getPixel(3,y);
    pixel->h = 120;
    pixel->l = 0.5;
    pixel->s = 1;
    pixel = inpng.getPixel(4,y);
    pixel->h = 58;
    pixel->l = 200;
    pixel->s = 1;
  }
  inpng.writeToFile("input-images/3x4-mytest.png");

  ImgList mylist(inpng);
  // ImgList copyList(mylist);
  mylist.Carve(1, 1);
  PNG outpng = mylist.Render(false, 0);
  outpng.writeToFile("input-images/soln-3x4-mytest.png");
}
